	...
	$.ajax({
        ...
        success: function (data) {
        	if($.isEmptyObject(data))
        		{
        		  swal("No Paymenttype present in List");
        		}
        	else
        	{
	        	appendList(data);
...
function appendList(data)
{
	...
	for (var i=0;i<data.length;i++)  {
		...
		if(i==0)
			{
				...
				$(".Id").val(data.paymentTypeId);
				if(activeStatus=="Active")
					$('.isActive').prop('checked', true);
				else
					$('.isActive').prop('checked', false);
				
				if(data[i].isRefundable=="Y")
					$('.refundable').prop('checked', true); 
				else
					$('.refundable').prop('checked', false);
				if(data[i].periodType=="Y")
					$('.periodType').prop('checked', true); 
				else
					$('.periodType').prop('checked', false);
		...
		$('#example').append('<li '+active+' onclick="showdetails('+data[i].paymentTypeId+')"> <a href="" class="" data-toggle="tab">'
		           +' <div class="col-md-12">'
		           +'  <label class="col-xs-6">'+data[i].name+'</label>'
		           +'   <label class="col-xs-6 mytext" data-paymenttypeid="'+data[i].paymentTypeId+'">'+data[i].name+'</label>'
		           +'  </div>'
		           +'  <div class="col-md-12">'
...
function showdetails(id)
{
	$.ajax({
        ...
        success: function (data) {
        	if($.isEmptyObject(data))
        		{
        		  swal("Paymenttype details is empty");
        		}
        	else
        	{
        		var activeStatus='';
