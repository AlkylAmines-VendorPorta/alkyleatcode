	...
	$.ajax({
        ...
        success: function (data) {
        	if($.isEmptyObject(data))
        		{
        		  swal("No DocumentType present in List");
        		}
        	else
        	{
	        	appendList(data);
...
function appendList(data)
{
	...
	for (var i=0;i<data.length;i++)  { 
		...
		$('#example').append('<li '+active+' onclick="showdetails('+data[i].documentTypeId+')"> <a href="" class="" data-toggle="tab">'
		           +' <div class="col-md-12">'
		           +'  <label class="col-xs-6">'+data[i].name+'</label>'
		           +'   <label class="col-xs-6 mytext" data-Id="'+data[i].documentTypeId+'">'+data[i].description+'</label>'
		           +'  </div>'
		           +'  <div class="col-md-12">'
...
function showdetails(id)
{
	...
	$.ajax({
        ...
        success: function (data) {
        	if($.isEmptyObject(data))
        		{
        		  swal("DocumentType details is empty");
        		}
        	else
        	{
	        	var activeStatus='';
