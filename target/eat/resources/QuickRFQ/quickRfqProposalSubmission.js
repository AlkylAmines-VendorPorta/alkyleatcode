...
$(document).ready(function(){
	...
	$("#tahdrDetailsTab").on("click",function(){
		...
		getTahdrDetails();
		$(this).removeAttr("disabled");
		/*var resp=fetchList("getPurchasedTenderByTypeCode",$("input[name='toggleTenderType']").val());
	...
	var li=	'<li tahdrDetailId="'+value.tahdr.tahdrDetail[0].tahdrDetailId+'" bidderId="'+value.bidderId+'" class="tahdr '+active+'" onclick="onClickTahdr(this);" >'+
			   	'<a data-toggle="tab">'+
			        ...
			        '<div class="col-md-12">'+
			            ...
			            '<label class="col-xs-6">'+formatDate(value.tahdr.tahdrDetail[0].technicalBidToDate)+'</label>'+
			        '</div>'+
			    '</a>'+
		    '</li>';
	return li;
...
function loadTenderDetailsRightPane(bidder){
	...
	if(!$.isEmptyObject(bidder)){
		...
		var tahdrId=bidder.tahdr.tahdrId==null?'':bidder.tahdr.tahdrId;
		var isCommercialDocs=bidder.tahdr.tahdrDetail[0].isCommercialDocs;
		var isTechnicalDocs=bidder.tahdr.tahdrDetail[0].isTechnicalDocs;
		var isGtp=bidder.tahdr.tahdrDetail[0].isGTP;
		var isPriceDocs=bidder.tahdr.tahdrDetail[0].isPriceDocs;
		var commencementWithin=bidder.tahdr.tahdrDetail[0].commencementPeriod;
		$("#deliveringMonth").val(commencementWithin);
		tenderTypeCode=tahdrTypeCode;
		$('#tahdrDoc').data('id',tahdrId);
		...
		tenderScopeSetUp(isCommercialDocs,isTechnicalDocs,isGtp,isPriceDocs);
	}
	setChildLoadFlag(true);
}
	...
	if($.isEmptyObject(paymentDetail)){
		...
		if(faApproved=='Y'){
			...
			if(paymentDetail.bankGuaranteeAttachment!=null){
				var bgId = paymentDetail.bankGuaranteeAttachment.attachmentId == null ? '': paymentDetail.bankGuaranteeAttachment.attachmentId;
				var bgName = paymentDetail.bankGuaranteeAttachment.fileName == null ? '': paymentDetail.bankGuaranteeAttachment.fileName;
				$("#BGAttachment").val(bgId);
				var url=$("#a_bgAttachment").data('url');
				$("#a_bgAttachment").attr('href',url+'/'+bgId);
...
function getTahdrDetails(){
	if(getChangedFlag()){
		var toggleValue=$("input[name='toggleTenderType']").val();
...
function loadTenders(){
	debugger;
	if(getChangedFlag()){
		var response=fetchList("getQuickRFQ",$("#documentType").val());
...
function tenderScopeSetUp(isCommercialDocs,isTechnicalDocs,isGtp,isPriceDocs){
	if(isTechnicalDocs=='N' && isGtp=='N'){
		$('.technicalBidClass').attr('style','display : none ;');
	}else{
		$('.technicalBidClass').removeAttr('style');
		if(isTechnicalDocs=='Y'){
			$('.technicalDocsClass').removeAttr('style');
		}else{
			$('.technicalDocsClass').attr('style','display : none ;');
		}
		if(isGtp=='Y'){
			$('.gtpClass').removeAttr('style');
		}else{
			$('.gtpClass').attr('style','display : none ;');
		}
	}
	if(isCommercialDocs=='Y'){
		$('.commercialDocsClass').removeAttr('style');
	}else{
		$('.commercialDocsClass').attr('style','display : none ;');
	}
	
	if(isPriceDocs=='Y'){
		$('.priceDocsClass').removeAttr('style');
	}else{
		$('.priceDocsClass').attr('style','display : none ;');
	}
}