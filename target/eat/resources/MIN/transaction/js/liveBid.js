...
function loadAuctionRightPane(data) {
	if (!$.isEmptyObject(data)) {
		...
		$('#tenderDetailsForm #department').html(department);
		...
		setHeaderValues("Tender No.: " + tenderCode, 
				"Remaining Time: " 
				, "Contact EmailId : " + contactEmail,
				"Description : " + description);
		$('.rightTopClass').css("color","red");
		$('.rightTopClass').append("<label class='rightTopClassTimer'></label>");
		$('.rightTopClassTimer').append().timer({
		    countdown: true,
		    duration: formateCurrentDate(bidEndDate),
		    callback: function() {
		    	$('.rightTopClassTimer').html('Times Up !');
		    	window.reload();
		    }
		});
...
function loadAuctionMaterialRightPane(data,rank) {
	...
	if (!$.isEmptyObject(data)) {
		...
		setHeaderValuesWithMoreParam("Material Name.: " + name, "Remaining Time: ", "Base Price Rate (in Rs): "+baseRateValue ,
		...
		setHeaderValuesWithMoreParam("Material Name.: ", "Remaining Time: ", "Base Price Rate (in Rs): " ,
...
function resetToPreviousBid(event){
	event.preventDefault();  
	var tahdrId= $('#tahdrForm #tahdrId').val();
	var tahdrMaterialId=$('.leftPaneData').find('li.active').attr('id');
	submitToURL('autoRefreshData/'+tahdrId+'/'+tahdrMaterialId,'autoRefreshDataView');
