...
$(document).ready(function(){
	showButtonByRole();
	showReceiptByRole();
	setVendorPaymentType($("#isManufacturer").val(),$("#isTrader").val(),$("#isInfra").val());
	$('#addPaymentBtnId').click(function(event) {
		...
		$("#infraPayment").removeAttr('readonly','readonly');
		setVendorPaymentType($("#isManufacturer").val(),$("#isTrader").val(),$("#isInfra").val());
		...
		$(".approvalFields").css('display','none');
	});

	$('input[type=radio][name=paymentMode]').change(function(){
		/*showFieldForPaymentMode($(this).val());*/
		if($(this).val()=="OP"){
	...
	$('#cancelPaymentBtnId').click(function(event) {
	 ...
	 $("#payOnlinePtnrRegBtn").click(function(event){
	    	...
	    	if(validate){
	    		submitIt("RegonlnPmntForm","validateOnlinePayment","onlinePaymentValidateResp");
...
function showFieldForPaymentMode(mode){
	if(mode=="OP"){
		  $(".onlinePaymentFields").find("input").attr('disabled','disabled');
		  $(".hidePaymentFields").hide();
		 /* $("#savePaymentBtnId").hide();*/
...
function onlinePaymentValidateResp(data){
...
function setVendorPaymentType(isManufacturer,isTrader,isInfra){  
	  
	/*var isManufacturer=$("#isManufacturer").val(); 
    var isTrader=$("#isTrader").val();
    var isInfra=$("#isInfra").val();*/
	$("label[for=traderPayment],#traderPayment").attr('disabled','disabled');
	$("label[for=manufacturerPayment],#manufacturerPayment").attr('disabled','disabled');
	$("label[for=infraPayment],#infraPayment").attr('disabled','disabled');
	$("#traderPayment").prop('checked',false);
	$("#manufacturerPayment").prop('checked',false);
	$("#infraPayment").prop('checked',false);
	var isChecked=false;
	if(isTrader=="Y"){
		$("label[for=traderPayment],#traderPayment").removeAttr('disabled','disabled');
		isChecked=true;
		$("#traderPayment").prop('checked',true);
	}
	if(isManufacturer=='Y'){
		$("label[for=manufacturerPayment],#manufacturerPayment").removeAttr('disabled','disabled');
		if(!isChecked){
			isChecked=true;
	...
	if(isInfra=="Y"){
		$("label[for=infraPayment],#infraPayment").removeAttr('disabled','disabled');
		if(!isChecked){
			isChecked=true;
			$("#infraPayment").prop('checked',true);
		}
	}
	/*if(isManufacturer=='Y' && isTrader=="Y" && isInfra=="Y")
    {
		$("#manufacturerPayment").show();
		$("#traderPayment").show();
		$("#traderPayment").prop('checked',true);
		$("#manufacturerPayment").prop('checked',false);
		$("#infraPayment").show();
			  ...
			  if(!$.isEmptyObject(data.objectMap.partner))
				  {
				     ...
				     setVendorPaymentType(partner.isManufacturer,partner.isTrader,partner.isInfra);
				  }
			}
...
function changeFormFieldValue(el){
	...
	if(type=='RG'){
		 code='RG';
		 var paymentData=paymentTypeArray["paymentType"+code];
		 setTaxesByPayemntType(paymentData);
...
function setTaxesByPayemntType(data){
	if(!isEmpty(data)){
		$("#partnerOrgPaymentForm #gstRate").text(data.gst);
		$("#partnerOrgPaymentForm #amount").val(data.amount);
		var gstAmount=((data.amount*data.gst)/100);
...
function appendPartnerOrgPaymentDetailData(value,active){
	...
	if(!isEmpty(value)){
		...
		var paymenttype = value.paymentType==null?'':value.paymentType.paymentTypeId;
		var paymenttypeName = value.paymentType==null?'':value.paymentType.name;
...
function loadPartnerOrgPaymentRightPane(data){
	...
	var paymenttype = data.paymentType==null?'':data.paymentType.paymentTypeId;
	...
	var moneyReceiptNumber=data.moneyReceiptNumber==null?'':data.moneyReceiptNumber;
	var moneyReceiptDate=data.moneyReceiptDate==null?'':formatDate(data.moneyReceiptDate);
	var faApprovedDate=data.faApprovedDate==null?'':formatDate(data.faApprovedDate);
	var foApprovedDate=data.foApprovedDate==null?'':formatDate(data.foApprovedDate);
	...
	$("#moneyReceiptNumber").val(moneyReceiptNumber);
	$("#moneyReceiptDate").val(moneyReceiptDate);
	$("#faApprovedDate").val(faApprovedDate);
	...
	showFieldForPaymentMode(paymentmode);
	if(isFAApproved=='Y' || isFAApproved=="N" || isFOApproved=="Y" || isFOApproved=="N"){
		$("#partnerOrgPaymentForm .disableBtn").attr('disabled','disabled');
		var partnerData=$("#partnerData").val();
		if(isFAApproved=='Y' && partnerData=="partnerRegistration"){
		  $("#partnerOrgPaymentForm #payReceipt").show();
		  $("#partnerOrgPaymentForm #paymentReceiptBtnId").css('display','block');
		  $(".approvalFields").css('display','block');
		}
	}else{
		$("#partnerOrgPaymentForm .disableBtn").removeAttr('disabled','disabled');
		$("#partnerOrgPaymentForm #payReceipt").hide();
...
function setPaymentType(vendorTypePayment)
{
	if(vendorTypePayment=='TP')
		{
			...
			$("#manufacturerPayment").prop('checked',true);
			$("#infraPayment").prop('checked',false);
		}else if(vendorTypePayment=='INFP'){
			$("#traderPayment").prop('checked',false);
			$("#manufacturerPayment").prop('checked',false);
			$("#infraPayment").prop('checked',true);
		}
	$("#traderPayment").attr('readonly','readonly');
	$("#manufacturerPayment").attr('readonly','readonly');
	$("#infraPayment").attr('readonly','readonly');
   ...
   if(isFOApproved=="Y" || isFOApproved=="N" || isFAApproved=="Y" || isFAApproved=="N" )
	   {
	     ...
	     $("#partnerOrgPaymentForm .disableBtn").attr('disabled','disabled');
	     $("#partnerOrgPaymentForm .partnerTabs").addClass('readonly');
	   }else{
