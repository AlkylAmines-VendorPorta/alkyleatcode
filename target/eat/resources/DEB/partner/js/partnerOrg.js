...
$(document).ready(function() {
	...
	$('#manPower').on('keyup',function(event) {
		...
		if(manpower<10)
		{
		    $("#factoryLicenseCopyLabel").html("<label>Attach Undertaking On Company Letter Head<span class='red'>*</span></label>");
...
function partnerOrgResp(data) {
	...
	$('#partnerOrgForm #partnerOrgLocationId').val(locationID);
...
function onPartnerOrgTabLoad(data) {
	...
	if (data.objectMap.hasOwnProperty('orgs')) {
		if (!isEmpty(data.objectMap.orgs)) {
			...
			$("#factoryInspectectionTabId").hide();
			$('.disableFactoryTabs').hide();
			$(".leftPaneData").html("");
			$('#partnerOrgForm')[0].reset();
			$('#partnerOrgId').val("");
...
function appendPartnerOrgDetail(value, active) {
	var leftPanelHtml = '';
	var partnerOrgId = value==null?'':value.partnerOrgId == null ? '' : value.partnerOrgId;
	if (active) {
		leftPanelHtml = leftPanelHtml
				+ ' <li class="active" onclick="showOrgDetail(' + partnerOrgId+ ')" id="' + partnerOrgId + '">';
	} else {
		leftPanelHtml = leftPanelHtml + ' <li onclick="showOrgDetail('+ partnerOrgId + ')" id="' + partnerOrgId + '">';
	}
	var factoryName = value==null?'':value.name == null ? '' : value.name;
	var estdDate = value==null?'':value.estdDate == null ? '' :formatDate(value.estdDate);
	var address1 = value==null?'':value.location.address1 == null ? '': value.location.address1;
	var inspectionReportNo = value==null?'':value.inspectionReportNo == null ? ''
			: value.inspectionReportNo;
	var postal = value==null?'':value.location.postal == null ? '' : value.location.postal;
	var country = value==null?'':value.location.country == null ? ''
			: value.location.country.countryId == null ? ''
					: value.location.country.countryId;
	var region = value==null?'':value.location.region == null ? ''
			: value.location.region.regionId == null ? ''
					: value.location.region.regionId;
	var city = value==null?'':value.location.city == null ? '' : value.location.city;
	var district = value==null?'':value.location.district == null ? ''
			: value.location.district.districtId == null ? ''
					: value.location.district.districtId;
	var partnerOrgLocationId = value==null?'':value.location.locationId == null ? ''
			: value.location.locationId;
	var licenceNo = value==null?'':value.licenceNo == null ? '' : value.licenceNo;
	var licenceValidityDate = value==null?'':value.licenceValidityDate == null ? ''
			: formatDate(value.licenceValidityDate);
	var licenseType = value==null?'':value.licenseType == null ? '' : value.licenseType;
	var licenceCopyId = value==null?'':value.licenceCopy == null ? ''
			: value.licenceCopy.attachmentId == null ? ''
					: value.licenceCopy.attachmentId;
	var licenceCopyName = value==null?'':value.licenceCopy == null ? ''
			: value.licenceCopy.fileName == null ? ''
					: value.licenceCopy.fileName;
	var machinaryListCopyId = value==null?'':value.machinaryListCopy == null ? ''
			: value.machinaryListCopy.attachmentId == null ? ''
					: value.machinaryListCopy.attachmentId;
	var machinaryListCopyName = value==null?'':value.machinaryListCopy == null ? ''
			: value.machinaryListCopy.fileName == null ? ''
					: value.machinaryListCopy.fileName;
	var inspectionCopyId = value==null?'':value.inspectionReportCopy == null ? '': value.inspectionReportCopy.attachmentId == null ? '': value.inspectionReportCopy.attachmentId;
	var inspectionCopyName = value==null?'':value.inspectionReportCopy == null ? '': value.inspectionReportCopy.fileName == null ? '': value.inspectionReportCopy.fileName;
	var inspectionDate = value==null?'':value.inspectionDate == null ? '' :formatDate(value.inspectionDate);
	
	var manpower = value==null?'':value.manPower == null ? '' : value.manPower;
	...
	var staffListId = value==null?'':value.staffListCopy == null ? '': value.staffListCopy.attachmentId == null ? '': value.staffListCopy.attachmentId;
	var staffListCopyName = value==null?'':value.staffListCopy == null ? '': value.staffListCopy.fileName == null ? '': value.staffListCopy.fileName;
	
	var factoryInspected= value==null?'':value.isFactoryInspected == null ? '': value.isFactoryInspected;
	var paymentId=value==null?'':value.paymentDetail==null?'':value.paymentDetail.paymentDetailId==null?'':value.paymentDetail.paymentDetailId;
	var factoryStatus="";
	var factoryLabelClass="";
	var commonData=setOrgStatus(isEEApproved,isCEApproved);
	factoryStatus=commonData.fStatus;
	factoryLabelClass=commonData.fClass;
	leftPanelHtml = leftPanelHtml + ' <a href="#Master" data-toggle="tab">'
			+ ' <div class="col-md-12">'
			+ '  <label class="col-xs-6" id="labelFactoryName-' + partnerOrgId+ '">'+factoryName+'</label>'
			+ '  <label class="col-xs-6 '+factoryLabelClass+'" >'+factoryStatus+'</label>'
			+ ' </div>'
			+ ' </a>' + ' </li>';
...
function setOrgStatus(isEEApproved,isCEApproved){
	
	var factoryStatus="";
	var factoryClass="";
	if(isCEApproved=="C"){
		factoryStatus="CE Clarified";
		factoryClass="statusclerify";
	}else if(isCEApproved=="Y"){
		factoryStatus="CE Approved";
		factoryClass="statusCEApproved";
	}else if(isEEApproved=="Y"){
		factoryStatus="EE Approved";
		factoryClass="statusEEApproved";
	}else if(isEEApproved=="C"){
		factoryStatus="EE Clarified";
		factoryClass="statusclerify";
	}else if(isEEApproved=="" && isCEApproved==""){
		factoryStatus="Pending";
		factoryClass="statuspending";
	}
	return {fStatus:factoryStatus, fClass:factoryClass};
}
...
function loadPartnerOrgRightPane(data) {
	...
	loadStateByCountry('country','partnerOrgForm','region');
	$("#partnerOrgForm #region").val(region);
	loadDistrictByState('region','partnerOrgForm','district');
	...
	$('#partnerOrgForm').find('input,select').removeClass("errorinput");
	showOrgValidityDate(data);
}
function showOrgValidityDate(data){
	debugger;
	$("#orgExpireOn").html("");
	$("#orgExpiredOn").html("");
	var orgValidTo=data==null?'':data.validTo==null?'':new Date(data.validTo).setHours(0,0,0,0);
	var today=new Date().setHours(0,0,0,0);
	var partnerData =  $("#partnerData").val();
	if(partnerData=="partnerRegistration" && orgValidTo!='')
		{
			if(orgValidTo>today || orgValidTo==today){
				$("#orgExpireOn").css('display','block');
				$("#orgExpiredOn").css('display','none');
				$("#orgExpireOn").html("Expire On:"+formatDate(orgValidTo));
			}else if(orgValidTo<today){
			    $("#orgExpiredOn").css('display','block');
			    $("#orgExpireOn").css('display','none');
			    $("#orgExpiredOn").html("Expired On:"+formatDate(orgValidTo));
			}
		}else{
		    $("#orgExpireOn").css('display','none');
			$("#orgExpiredOn").css('display','none');
...
function changeButtonPropertiesByOrgStatus(data)
{
	 ...
	 if(partnerData=="partnerRegistration")
		{
		 var partnerStatus=data==null?'':data.partner==null?'':data.partner.status==null?'':data.partner.status;
		 var partnerIsEEApproved=data==null?'':data.partner==null?'':data.partner.isEEApproved==null?'':data.partner.isEEApproved;
		 if(data.partner!=null && (partnerStatus=='CO' || partnerStatus=='EEC'||  partnerStatus=='EDIT'|| (partnerStatus=='DR' && (isEEApproved!="" || isCEApproved!=""))))
			  {
			  ...
			  }else if(data.partner!=null && (partnerStatus=='RJ' || partnerStatus=='CEC' || partnerIsEEApproved=='Y' || partnerStatus=='IP')){
				  $("#addFactoryBtnId").addClass("readonly");
				  $("#deleteFactoryBtnId").addClass("readonly");
				  $("#editFactoryBtnId").hide();
				  $(".partnerOrgCEApproveDiv").show();
				  $(".partnerOrgEEApproveDiv").show();
			   }else if(partnerStatus==null || partnerStatus=="" || partnerStatus=='DR')
				 {
				    $("#editFactoryBtnId").hide();
				    $(".partnerOrgTabs").removeClass("readonly");
				    $(".partnerOrgActionBtn").removeClass("readonly");
