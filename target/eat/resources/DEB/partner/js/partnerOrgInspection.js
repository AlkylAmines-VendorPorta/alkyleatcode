        		 ...
        		 if(data.objectMap.partnerOrg.isFactoryInspected=="N")
    		    	{
    		    	   ...
    		    	   $("#inspectionLabelDiv").show();
       		    	   $("#inspectionLabel").html("Factory inspection is not done");
    		    	}else{
    		    		$("#orgInspectionForm .orgInspecitonField").removeClass("readonly");
    		    		$("#inspectionLabelDiv").hide();
        		    	$("#inspectionLabel").html("");
    		    	}
           	}
        }
...
function loadPartnerOrgInspectionRightPane(data) {
	...
	if(!isEmpty(data)){
	    ...
	    var isInspectionDone=data.partnerOrg==null?'':data.partnerOrg.isFactoryInspected==null?'':data.partnerOrg.isFactoryInspected;
	    if(isInspectionDone=="N"){
	    	$("#inspectionLabelDiv").show();
	    	$("#inspectionLabel").html("Factory inspection is not done");
	    }else{
	    	$("#inspectionLabelDiv").hide();
