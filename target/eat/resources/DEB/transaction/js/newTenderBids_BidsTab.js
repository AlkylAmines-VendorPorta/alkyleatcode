...
function getTechnicalBid(){
	...
	setLeftPaneHeader("Technical Bid", $("#leftPaneData li").length);
}
...
function getPriceBid(){
	...
	setLeftPaneHeader("Price Bid", $("#leftPaneData li").length);
}
...
function getCommercialBid(){
	...
	setLeftPaneHeader("Commercial Bid", $("#leftPaneData li").length);
}
...
function getBids_PriceBid(){
	...
	if(tahdrMaterialId!=null && partnerId!=null){
		...
		Alert.warn("#itemDetailForm #tahdrMaterialId OR #bidderDetailForm #bidderId is NULL");
	}
}

function getBids_PartsPriceBid(){
	
	var tahdrMaterialId=$("#itemDetailForm #tahdrMaterialId").val();
	var partnerId=$("#bidderDetailForm #partnerId").val();
	if(tahdrMaterialId!=null && partnerId!=null){
		submitToURL("getPartnerPriceBidForParts/"+tahdrMaterialId+"/"+partnerId,"getBids_PartsPriceBidResp");
	}else{
		Alert.warn("#itemDetailForm #tahdrMaterialId OR #bidderDetailForm #bidderId is NULL");
	}
}

function getBids_PartsPriceBidResp(data){
	var priceBid=renderPartsTab(data);
	if(!$.isEmptyObject(priceBid)){
		...
		$("#partsPriceBidForm .priceBidId").val(priceBidId);
		$("#partsPriceBidForm .materialSpecificationId").val(materialSpecificationId);
		$("#partsPriceBidForm #priceBidId").val(priceBidId);
		$("#partsPriceBidForm #offeredQuantity").val(offeredQuantity);
		$("#partsPriceBidForm #exGroupPriceRate").val(exGroupPriceRate);
		$("#partsPriceBidForm #freightChargeRate").val(freightChargeRate);
		$("#partsPriceBidForm #ticRate").val(ticRate); //transit insurance charges
		$("#partsPriceBidForm #taxRate").val(taxRate);
		
		$("#partsPriceBidForm #igstLable").text(igst);
		$("#partsPriceBidForm #cgstLable").text(cgst);
		$("#partsPriceBidForm #sgstLable").text(sgst);
		
		$("#partsPriceBidForm #igst").val(igst);
		$("#partsPriceBidForm #cgst").val(cgst);
		$("#partsPriceBidForm #sgst").val(sgst);
		
		$("#partsPriceBidForm #taxAmount").val(taxAmount);
		$("#partsPriceBidForm #fddRate").val(fddRate);
		$("#partsPriceBidForm #fddInWords").val(fddInWords);
		$("#partsPriceBidForm #isConfirmed").val(isConfirmed);
		
		$("#partsPriceBidForm #totalExGroupPrice").val(totalExGroupPrice);
		$("#partsPriceBidForm #totalFreightCharges").val(totalFreightCharges);
		$("#partsPriceBidForm #totalTic").val(totalTic);
		$("#partsPriceBidForm #fddRate").val(fddRate);
		$("#partsPriceBidForm #fddAmount").val(fddAmount);
		$("#partsPriceBidForm #taxAmount").val(taxAmount);
		$("#partsPriceBidForm #totalTax").val(totalTax);
		$("#partsPriceBidForm #igstAmount").val(igstAmount);
		$("#partsPriceBidForm #cgstAmount").val(cgstAmount);
		$("#partsPriceBidForm #sgstAmount").val(sgstAmount);
		$("#partsPriceBidForm #fddRateGST").val(fddRateGST);
		$("#partsPriceBidForm #fddAmountGST").val(fddAmountGST);
		$("#partsPriceBidForm #fddAmountWithGSTInWords").val(amountInWords);
		$("#partsPriceBidForm #fddAmountWithGSTInWordsLabel").attr("title",amountInWords);
		$("#partsPriceBidForm #fddAmountWithGSTInWordsLabel").text(amountInWords);
	}
}

function renderPartsTab(data){
	var tabHtmlContent='';
	var count=0;
	var partsPriceBidData='';
	$.each(data.objectMap.partsPriceBidList,function(key,value){
		if(count==0){
			partsPriceBidData=value;
		}
	tabHtmlContent+=tabHtmlContent+'<h4 class="panel-title">'+
         '<a onclick="getBids_PartsPriceBid(this)" data-specification_id="'+value.materialSpecification.materialSpecificationId+'" data-toggle="collapse" data-parent="#accordion22" href="#PartsPriceFilledData">Parts Price Filled Data</a>'+
       '</h4>';
	});
	$("#PartsPriceDivId").html(tabHtmlContent);
	return partsPriceBidData;
}
