...
$(document).ready(function(){
	...
	$("#customDocsDiv").on("change",".attachment",function(){
		uploadFile(this);
		var idCode=$(this).data('id').split("_");
		$("#"+idCode[1]).removeAttr("disabled");
		var dataName=$("#anchor_"+idCode[1]).html();
		$("#"+idCode[1]).attr("data-name",dataName);
	});
});
...
function getStdCustDosc(){
	...
	if(getChangedFlag()){
		...
		if(id!=''){
			...
			var tahdrDetailData =fetchList("getTAHDRDates",id);
			if(!$.isEmptyObject(tahdrDetailData)){
				loadTahdrDetailDataToLeftPane(tahdrDetailData.objectMap.tahdrDate);
			}
			...
			if (tenderStatus == 'Drafted'){
				...
				/*$('.stdCustDocDiv').removeClass('readonly');*/
				...
				/*$('.stdCustDocDiv').addClass('readonly');*/
				$('#saveStdCstDocForm .save').hide();
				$('#saveStdCstDocForm .cancel').hide();
			}
			populateTenderDoc(tahdrDetailStdDocs,tahdrDetail);
...
function populateTenderDoc(standardCustomDoc,tahdrDetail){
	...
	addNewCMDiv();
	var activeStatus=tahdrDetail==null?'':tahdrDetail.isActive==null?'':tahdrDetail.isActive;
	if(activeStatus!=null){
		if(activeStatus=='Y'){
	 	...
	 	setLeftPaneHeader("Standard Docs", null);
	setChildLoadFlag(true);
}
...
function populateCustomDocs(custDoc){
    if(!$.isEmptyObject(custDoc)){
    	...
    	var html='<div class="form-group stdCustDocDiv">'
   		 ...
   		 +'<input type="checkbox" '+dis+' value="Y" class="standardCustomDoc standardCustomDocuments2" id="'+custDoc.code+'" data-value="'+count+'" data-name="'+attName+'">' /*data-name="Custom Document '+custDoc.code.split('CM')[1]+'"*/
   		...
   		+'<input type="file" data-id="attachment_'+custDoc.code+'" data-async="N" data-anchor="anchor_'+custDoc.code+'" class="attachment form-control  standardCustomDocuments2" value="'+next+'">'
...
function addNewCMDiv(){
	...
	var html='<div class="form-group stdCustDocDiv">'
		 ...
		 +'</div>'
		 +'<div class="col-sm-5 col-xs-6">'
		 +'<input type="file" data-id="attachment_CM'+customDocCount+'" data-async="N" data-anchor="anchor_CM'+customDocCount+'" class="attachment form-control  standardCustomDocuments2" value="'+next+'">'
...
function loadTahdrDetailDataToLeftPane(tahdrData){
	...
	var firstRow = null;
	/*tahdrDetailArray=[];*/
	$.each(Object.values(tahdrData),function(key,tahdrDetail){
