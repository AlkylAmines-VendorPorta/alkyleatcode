	...
	//cancel
			...
			$("#payReceiptLinkId").on("click",function(event){
				...
				if(paymentType=='Tender/Auction Fee'){
					paymentType='TF'
					var tahdr=$("#paymentdetailId").val();
					anchorHtml="invoice?id="+tahdr+"&paymentType="+paymentType;
				}
				else if(paymentType=='EMD'){
					paymentType='EMD'
					var EMD=$("#paymentdetailId").val();
					anchorHtml="invoice?id="+EMD+"&paymentType="+paymentType;
				}
				else if(paymentType=='Registration Fee'){
					paymentType='RG'
					var RGId=$("#paymentdetailId").val();
					anchorHtml="invoice?id="+RGId+"&paymentType="+paymentType;
				}
				else if(paymentType=='Registration Renewal Fee'){
					paymentType='RN'
					var RNId=$("#paymentdetailId").val();
					anchorHtml="invoice?id="+RNId+"&paymentType="+paymentType;
				}
...
function renderList(){ 
	paymentStatus='';
	var data = fetchPaymentApprovalList(1, leftPanePageSize, 'none', 'none');
	$('.dateSetter').removeClass('readonly');
	getPagePaymentResp(data);
	setPagination(data.objectMap.LastPage, selectedPage , maxVisiblePageNumbers);	
...
function loadPaymentDetailToRightPane(payment){
	...
	if(!$.isEmptyObject(payment)){
		...
		var partnerName=payment.partner==null?'':payment.partner.name==null?'':payment.partner.name;
		var partnerOrgName=payment.partnerOrg==null?'':payment.partnerOrg.name==null?'':" - "+payment.partnerOrg.name;
		var tahdrId=payment.tahdr==null?'':payment.tahdr.tahdrId==null?'':payment.tahdr.tahdrId;
		if(payment.vendorTypePayment!=null){
			var vendorTypePayment=payment.vendorTypePayment==null?'':payment.vendorTypePayment;	
			if(vendorTypePayment=="TP")
        	{
		...
		var paymentTypeCode=payment.paymentType.code==null?'':payment.paymentType.code;
		var paymentMode=payment.paymentMode==null?'':payment.paymentMode;
		var amount=payment.amount==null?'':payment.amount;
        ...
        var validityDate=payment.validityDate==null?'':formatDate(payment.validityDate);
		...
		if(paymentMode=='DD'){
			...
			if(payment.bankGuaranteeAttachment!=null){
				var bgId = payment==null?'':payment.bankGuaranteeAttachment == null ? '': payment.bankGuaranteeAttachment.attachmentId == null ? '': payment.bankGuaranteeAttachment.attachmentId;
				var bgName = payment==null?'':payment.bankGuaranteeAttachment == null ? '': payment.bankGuaranteeAttachment.fileName == null ? '': payment.bankGuaranteeAttachment.fileName;
			...
			$('.BGFields').show();
			$('.ddFields').show();
			$('.attchmentaftershow').show();
			$(".micrCode").hide();
			$('#paymentDate').text('Issue Date');
			$('#paymentNo').text('Bank Gurantee No');
		}
		else{
			$('#paymentMode').val(paymentMode);
			$('.ddFields').hide();
			$('.BGFields').hide();
			$(".micrCode").hide();
			$('.attchmentaftershow').css('display', 'none');
		}
		$('#paymentMode').val(paymentMode);
		$("#amount").val(amount);
		$("#gstin").val(gstin);
