					...
					$.each(data,function(key,value){
						if(!$.isEmptyObject(value))
							...
							if(tahdrDetail!=undefined){
								...
								+'	<label class="col-xs-6" id="'+tahdrId+'_emdFee">'+emdFee+'</label>'
								+' </div>'
							    +' </a>'
							    +' </li>';
								i++;
...
function loadTahdrDetailRightPane(data){
	debugger;
		...
		setSchedulingDropDown(status,bidType);
}
...
function setSchedulingDropDown(status,bidType){
	debugger;
	$("#openingDateTime").val('');
	$("#closingDateTime").val('');
