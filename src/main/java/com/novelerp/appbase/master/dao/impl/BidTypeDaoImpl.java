 ...
 */
@Repository
public class BidTypeDaoImpl extends AbstractJpaDAO<BidType, BidTypeDto> implements BidTypeDao{

private static final String ENTITY_NAME="M_BIDTYPE";
	
	@PostConstruct
	private void init() {
		setClazz(BidType.class, BidTypeDto.class);
	}
	
	@Override
	@Transactional(propagation =Propagation.SUPPORTS)
	public List<BidType> findAll() {		 
		StringBuilder jpql = getBasicQuery();
		jpql.append(" WHERE ")
		.append(getActiveClause())
		.append(getBasicOrderBy());		
		EntityManager em = getEntityManager(); 
		return em.createQuery(jpql.toString(),BidType.class).getResultList();
	}
