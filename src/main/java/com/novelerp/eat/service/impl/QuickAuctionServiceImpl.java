...
@Repository
public class QuickAuctionServiceImpl extends AbstractContextServiceImpl<TAHDR, TAHDRDto> implements QuickAuctionService {
	...
	@PostConstruct
	private void init() {
		super.init(QuickAuctionServiceImpl.class, tahdrDao, TAHDR.class, TAHDRDto.class);
		setByPassProxy(true);
	}
	...
	@Override
	public List<TAHDRDto> getQuickAuctionList(Map<String, Object> map, int pageNumber, int pageSize,String searchColumn, String searchValue) {
		log.info("Get Quick Auction List");
	...
	public TAHDRDto updateQuickAuction(TAHDRDto tahdr,TAHDRDetailDto oldDto){
		if(tahdr !=null && tahdr.getTahdrDetail().iterator().hasNext()){
			...
			td.setTahdr(tahdr);
			td=tahdrDetailService.updateQuickAuction(td, oldDto);
			Set<TAHDRDetailDto> tdSet=new HashSet();
			tdSet.add(td);
			tahdr.setTahdrDetail(tdSet);
			return tahdr;
