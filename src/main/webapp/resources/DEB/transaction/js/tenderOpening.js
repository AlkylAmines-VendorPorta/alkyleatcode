	...
	if(!$.isEmptyObject(data)){
			...
			/*if(status=='TCOP'){
					...
					var count=1;
						...
						var downloadTechFile='<div class="col-sm-6"><a href="/eat/download/'+technicalDoc+'" id="downloadTechDoc">'+technicalDocName+'</a></div><div class="clearfix"></div>';
					...
					var count=1;
						...
						var downloadTechFile='<div class="col-sm-6"><a href="/eat/download/'+priceBidDoc+'" id="downloadTechDoc">'+priceBidDocName+'</a></div><div class="clearfix"></div>';
						formDataHtml+='<div class="col-sm-6"><label>'+count+'.'+materialName+' : </label></div>'+ downloadTechFile;
					...
					var count=1;
						...
						var downloadTechFile='<div class="col-sm-6"><a href="/eat/download/'+annexureDoc+'" id="downloadTechDoc">'+annexureDocName+'</a></div><div class="clearfix"></div>';
						formDataHtml+='<div class="col-sm-6"><label>'+count+'.'+materialName+' : </label></div>'+ downloadTechFile;
				...
				$('#tcclabelId').html('Download Commercial Deviation: ');
					...
					$.each(data.itemScrutinyList,function(key,value){
						if(value.scrutinyType=='TECHSCR'){
								...
								var downloadTechFile='<div class="col-sm-6"><a href="/eat/download/'+deviationDoc+'" id="downloadTechDoc">'+deviationDocName+'</a></div><div class="clearfix"></div>';
...
function showBidderBid(bidderId){
	...
	if(thadrBidType=='SB'){
		...
		loadBidderListRightPaneSB(bidderBidDetailArrayPB['bidder_'+bidderId]);
	}else{
		if(role=='VENADM'){
...
function loadBidderListForViewSB(data){
	...
	loadBidderListRightPaneSB(firstRow1)
	$('.example').paginathing({perPage: 6});
	}
function loadBidderListRightPaneSB(data1){
	debugger;
	$('.pagination').children().remove();
	if(!$.isEmptyObject(data1)){
			...
			$('#viewBidForm #tenderNo').html(tenderNo);
					...
					var count=1;
						...
						var priceBidDocName=data1.digiSignedDoc==null?'':data1.digiSignedDoc.fileName;
						if(priceBidDoc==''){
							var downloadTechFile='<div class="col-sm-6"><a href="#" id="downloadTechDoc">No File</a></div><div class="clearfix"></div>';	
						}else
						var downloadTechFile='<div class="col-sm-6"><a href="/eat/download/'+priceBidDoc+'" id="downloadTechDoc">PriceBid-'+priceBidDocName+'</a></div><div class="clearfix"></div>';
						formDataHtml+='<div class="col-sm-6"><label>'+materialName+' : </label></div>'
	...
	if(!$.isEmptyObject(data1) && !$.isEmptyObject(data2)){
			...
			$('#viewBidForm #tenderNo').html(tenderNo);
					...
					var count=1;
						...
						var downloadTechFile='<div class="col-sm-6"><a href="/eat/download/'+technicalDoc+'" id="downloadTechDoc">TechnicalBid-'+count+'</a></div><div class="clearfix"></div>';
						formDataHtml+='<div class="col-sm-6"><label>'+count+'.'+materialName+' : </label></div>'
						+ downloadTechFile;
					...
					var count=1;
						...
						var downloadTechFile='<div class="col-sm-6"><a href="/eat/download/'+priceBidDoc+'" id="downloadTechDoc">PriceBid-'+count+'</a></div><div class="clearfix"></div>';
						formDataHtml+='<div class="col-sm-6"><label>'+count+'.'+materialName+' : </label></div>'
			...
			if(!isEmpty(c1Bid)){
				...
				if(!isEmpty(c1Att)){
					...
					if(!isEmpty(c1Att)){
						...
						$("#downloadC1Doc").attr("href",c1Href+"/"+c1Att.attachmentId);
