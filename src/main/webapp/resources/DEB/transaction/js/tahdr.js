...
$(document).ready(function() {
			...
			$("#tenderBaseInfoTab").on('click',function(event) {
				...
				var documentType=$(".documentType").val();
				setLeftPaneHeader(documentType+" List", $("#leftPane li").length);
			});
			/*Onclick of first tab click*/
	...
	if(getChangedFlag()){
		...
		if(!isEmpty(response)){
			if(!isEmpty(response.data)){
				...
				if(!isEmpty(!isEmpty(response.objectMap) && !isEmpty(response.objectMap.LastPage))){
...
function addNewTahdr(event){
	...
	var documentType=$(".documentType").val();
	setHeaderValues("Code.: ", "Title : ", "Department : ", "Status : ");
}
...
*/	
	if (isEmpty(data) || isEmpty(data.objectMap) || isEmpty(data.tahdrData)) {
		return;
	}
	...
	var documentType=$(".documentType").val();
	setHeaderValues("Code.: " + tahdrCodes, "Title : " + title.substring(0, 100), "Department : " + department, "Status : " + tenderStatus);
}
...
function loadTahdrToRightPane(tahdr){
	if(!$.isEmptyObject(tahdr)){
		...
		$('#tahdrForm #tahdrStatus').val(getValue(tahdr.tahdrStatusCode));
		$('#TAHDRDetail #tahdrStatus').val(getValue(tahdr.tahdrStatusCode));
		...
		var documentType=$(".documentType").val();
		/*setHeaderValues("Code.: "+tahdrCodes, "Title : "+title, "Department : "+department, "Status : "+status);*/
		setHeaderValues("Code.: "+tahdrCodes, "Title : "+title.substring(0, 100), "Department : "+department, "Status : "+tenderStatus);
		if(roleDto.value=='VENADM' && tahdr.tahdrId!=null){
			...
			if(!isEmpty(bidderData) && !isEmpty(bidderData.objectMap)){
