...
function loadBiddersListForLiveBid(data) {
	...
	if (!$.isEmptyObject(data.objectMap.bidderList)) {
		...
		$.each(data, function(key, value) {
			...
			var rank=getRank;
			if(key==0){
				$('.listolivefbider tbody').append('<tr class="greenrow"><td onclick="showToolTip('+i+')"><span class="glyphicon glyphicon-plus-sign"></span></td><td>'+partnername+'</td>' + 
						'<td>'+rank+'</td>' + 
						/*'<td>'+factoryName+'</td>' + */
	            	...
	            	$('.listolivefbider tbody').append('<tr><td onclick="showToolTip('+i+')"><span class="glyphicon glyphicon-plus-sign"></span></td><td>'+partnername+'</td>' + 
						'<td>'+rank+'</td>' + 
						/*'<td>'+factoryName+'</td>' + */
						'<td>'+offeredQuantity+'</td>' + 
						'<td>'+exGroupPriceRate+'</td>' +
...
function showToolTip(count){
	...
	var igstPercent=bidderData.igst==null?0:bidderData.igst;
	var cgstPercent=bidderData.cgst==null?0:bidderData.cgst;
	var html= '<div id="tooltiptext_'+count+'"><h3>'+partnername+'<h4 class='+statusClass+'>('+rating+'/10)</h4></h3><ul>'+
        '<li><b>Factory Name:</b>'+factoryName+'</li><li><b>Per unit FDD:</b>'+fddRate+'</li><li><b>Total TIC:</b>'+totalTax+'</li><li><b>Per unit FDD With GST:</b>'+fddRateWithGST+'</li><li><b>FDD With GST:</b>'+fddAmountWithGST+'</li>'+
        '<li><b>Per Unit Ex-Works Price:</b>'+exGroupPriceRate+'</li>'+
    '</ul><i></i></div>';
