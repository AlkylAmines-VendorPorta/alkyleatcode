...
$(document).ready(function(){
	...
	$(".annexureBid").on("click",function(event){
		...
		if(!isEmpty(resp) && !isEmpty(resp.objectMap)){
			if(resp.objectMap.hasOwnProperty('isLowestPriceBid')){
				$.each(resp.objectMap.isLowestPriceBid,function(key,priceBid){
					var tahdrMaterialId=priceBid==null?'':priceBid.itemBid==null?'':priceBid.itemBid.tahdrMaterial==null?'':priceBid.itemBid.tahdrMaterial.tahdrMaterialId;
					if(tahdrMaterialId!=''){
						lowestPriceBidArray["lowestPriceBid_"+tahdrMaterialId]=priceBid;
					}
				});
			}
...
function appendTahdrLi(bidder,active){
	if(!isEmpty(bidder) && !isEmpty(bidder.tahdr)){
		  ...
		  if(!$.isEmptyObject(data.objectMap.priceBid)){
				...
				var value=data.objectMap.priceBid;
				  ...
				  var url=$("#a_annexurec1FileResponse").data('url');
					...
					$("#annexureC1Form #a_annexurec1FileResponse").prop('href', url+'/'+digiFile);
