...
$(document).ready(function(){
	...
	$('#cancelCommercialBid').on('click',function(event){
	 ...
	 $("#showInfo").click(function(){
		 $('#calculationDetailsH4').fadeIn(100).delay(1000).fadeOut(6000);
	  });
...
function createCalculatonMsg(data){
	var minqty=(materialQty*Number(data.minQuantity))/100;
	var s="Minimum Quantity Is "+minqty+"."
	$('#calculationDetails').text(s);
}

function populateCommercialBid(commercialBid){
	var commencementWithin=selectedTender.tahdr.tahdrDetail[0].commencementPeriod;
    createCalculatonMsg(selectedTender.tahdr.tahdrDetail[0]);
...
function onClickCommercialBid(){
	debugger;
	if(selectedTender.tahdr.tahdrTypeCode=='WT' || selectedTender.tahdr.tahdrTypeCode=='FA'){
		getCommercialBidDoc();
	}else{
		getCommercialBid();
	}
...
function deleteAttachmentCB(){
	var bidderId=$(".bidderId").val();
	submitToURL("deleteFinalCBDoc/"+bidderId,"respOfCBSubmit");
