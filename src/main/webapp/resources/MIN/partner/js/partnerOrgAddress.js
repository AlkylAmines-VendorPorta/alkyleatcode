function partnerOrgAddresstResp(data){
	swal(data.responseDto.message);	
}

function partnerOrgAddressTabClick(){
	submitToURL("getPartner", 'onPartnerOrgAddressTabLoad');	
}

function onPartnerOrgAddressTabLoad(data){
	debugger;
	$.each(data,function(key,value){
		if(key=='objectMap'){
			var responseMap = value;
			loadPartnerOrgAddressLeftPane(responseMap.companyContacts);
		}
...
function loadPartnerOrgAddressLeftPane(data){
	$(".leftPaneData").html("");
	var leftPanelHtml="";
	var i=0;
	var firstRow=null;
	$.each(data,function(key,value){
        ...
        +'	<label class="col-xs-6 ">'+lastName+'</label>'
        +' </div>'	
        +' <div class="col-md-12">'
        +'	<label class="col-xs-6">'+email+'</label>'
		+'	<label class="col-xs-6">'+mobileNo+'</label>'
		+' </div>'
        +' </a>'
        +' </li>';
		i = i++;
	...
	loadPartnerOrgAddressRightPane(firstRow);
}

function loadPartnerOrgAddressRightPane(data){
