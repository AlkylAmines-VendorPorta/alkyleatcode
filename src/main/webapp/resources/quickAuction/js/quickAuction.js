...
function createQuickAuction(event) {
	event.preventDefault();
	submitIt("tahdrForm", "createQuickAuction", "saveQuickAuctionResp");
}
...
function loadQuickAuctionToRightPane(tahdr){
	...
	if(!$.isEmptyObject(tahdr)){
		...
		if(!$.isEmptyObject(tahdrDetail)){
			...
			$("#tahdrForm #minBidDifference").val(getValue(tahdrDetail.minBidDifference));
		...
		setHeaderValues("No.: "+tahdrCodes, "Title : "+title.substring(0, 100), "Department : "+department, "Status : "+tenderStatus);
...
function addNewQuickAuction(event){
	...
	$('#tahdrForm .cancel').show();
	var documentType=$(".documentType").val();
	setHeaderValues("No.: ", "Title : ", "Department : ", "Status : ");
