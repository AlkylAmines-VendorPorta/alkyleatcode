...
function getTahdrDetails(){
	if(getChangedFlag()){
		...
		loadReferenceList("commencementPeriodMonth",detailList.objectMap.commencementPeriodValue);
		populateDesignation(detailList.objectMap.designations);
...
function loadTahdrDetailToRightPane(tahdrDetail){
	if(!$.isEmptyObject(tahdrDetail)){
		...
		$("#TAHDRDetail #tahdrFees").val(getValue(tahdrDetail.tahdrFees));
		calculateGST();
		...
		$("#isSetC1Later").val(formatDateTime(tahdrDetail.isSetC1Later));
		...
		$("#purchaseFromDate").val(formatDate(tahdrDetail.purchaseFromDate));
		$("#purchaseToDate").val(formatDate(tahdrDetail.purchaseToDate));
...
/*function calculateEMDFee(){
	...
	if($("#estimatedCost").val()==0){
		$("#tahdrFees").val(0);
		$("#gst").val(0);
		$("#totalFee").val(0);
	}
	else if($("#estimatedCost").val()>0 && $("#estimatedCost").val()<=100)
		{
		$("#tahdrFees").val(1000);
		calculateGST();
		}
...
function onChangeMonthDate(){
	...
	else if($("input[name='tenderTypeCodeToggle']:checked").val()=='PT'){
	...
	else if($("input[name='tenderTypeCodeToggle']:checked").val()=='FA')
		{
		if(ele<0 || ele==null || ele==undefined || ele==""){
		$("#MonthDate").css("display","none");
		$("#commencementPeriodMonth").removeClass('dropDown');
