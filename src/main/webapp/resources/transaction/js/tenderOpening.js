...
$("#autoRefresh").timer('remove');
	...
	if(!$.isEmptyObject(data.objectMap.tenderCommittee)){
	  ...
	  Alert.warn('No Tender/Auction Found !')
	  resetSearchForm();
	}
	...
	if(data!=null){
		if(role=='VENADM'){
			...
			if(!$.isEmptyObject(data.tahdrDetail[0])){
						...
						if(bidTypeCode=='SB' || tenderType=='FA' || tenderType=='PT'){
							$('#venderTenderOpeningSearchForm #vopeningType').val('PBOP');
					    ...
					    case 'TCOP':
					    	var openingDate = data.tahdrDetail[0].technicalBidFromDate==null?'':data.tahdrDetail[0].technicalBidFromDate;
					    ...
					    case 'PBOP':
					        ...
					        break;
					    case 'C1OP':
					    	var openingDate = data.tahdrDetail[0].c1FromDate==null?'':data.tahdrDetail[0].c1FromDate;
							var bidEndDate = data.tahdrDetail[0].c1ToDate==null?'':data.tahdrDetail[0].c1ToDate;
					        break;
					    case 'DBOP':
					    	var openingDate = data.tahdrDetail[0].deviationFromDate==null?'':data.tahdrDetail[0].deviationFromDate;
				...
				if(!$.isEmptyObject(data.tahdrDetail[0])){
							...
							thadrBidType=bidTypeCode;
							if(bidTypeCode=='SB' || tenderType=='FA' || tenderType=='PT'){
								$('#tenderOpeningSearchForm #openingType').val('PBOP');
	...
	if(!$.isEmptyObject(data.objectMap.bidderTenderList)){
		if(data.objectMap.bidderTenderList.length!=0){
			$.each(data.objectMap.bidderTenderList,function(key,value){
				if(value!=null){
				        value= value.tahdr;
					    /*var tahdrDetailId = value.tenderDetail==null?'': value.tenderDetail.tahdrDetailId;
						var tahdrCode = value.tenderDetail.tahdr.tahdrCode==null?'': value.tenderDetail.tahdr.tahdrCode;
						var title = value.tenderDetail.tahdr.title==null?'':value.tenderDetail.tahdr.title;
						var version  = value.tenderDetail.version==null?'':value.version;
						var emdFee 	  = value.tenderDetail.emdFee==null?'':value.tenderDetail.emdFee;
						var minQuatity = value.tenderDetail.minQuatity==null?'':value.tenderDetail.minQuatity;*/
...
/*function viewTenderBidResp(data)
{
	 
	$('.wizardTab').removeClass('readonly');
	$('#tenderOpeningForm .toggleTab').trigger('click');
	...
	if(!$.isEmptyObject(data.objectMap.bidderList)){
			...
			$.each(data.objectMap.bidderList,function(key,value){
				...
				fileDataHtml+='<div class="col-sm-12"><div class="col-sm-12 nopadding logspan"> <a class="col-sm-12 loglink nopadding"></a>'
		  		    +'<label class="col-sm-6 text-left nopadding">'+value.partner.name+'</label><label class="col-sm-6 text-right nopadding">'+status+'</label></div></div>';
...
function loadBidderListRightPane(data)
{
	...
	if(!$.isEmptyObject(data)){
		    ...
		    var status=data.itemBid==null?'':data.itemBid.bidder==null?'':data.itemBid.bidder.status==null?'':data.itemBid.bidder.status;
...
function bidAndBidderCount(openType,tenderId,status){
	...
	if(!$.isEmptyObject(data.objectMap.responseList)){
		...
		else if(openType=='PBOP'){
			debugger;
			if(status=='PU' || status=='DBSCH' || status=='PBSCH'){ 
				$.each(data.objectMap.responseList,function(key,value){
					debugger;
