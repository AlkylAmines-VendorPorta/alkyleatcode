...
$(document).ready(function(){
  ...
  /*  $('#leftPaneData').paginathing({perPage: 6});*/
	...
	$("#tenderDetailTab").on('click',function(event){
		...
		setHeaderValues(documentType+" Code:" +tenderCode, documentType+" Contact : "+tenderContact, "Description: "+description, "Department : "+tenderDepartment);
...
function downloadFinalTechnicalScrutinyPdf(){
...
function downloadFinalCommercialScrutinyPdf(){
	var bidderId=$('#bidderForm #bidderId').val();
	event.preventDefault();
	showLoader();
...
function loadTahdrDetailRightPane(data){
	 debugger;
		if(!$.isEmptyObject(data)){
				...
				//setHeaderValues("Tender No.: "+tahdrCode, "Tender Title : "+tenderNo, "Contact EmailId : "+contactEmailId, "Description : "+description);
...
function loadBidder(ele){
	...
	setHeaderValues("Partner Name: "+bidderName, "PAN Number : "+bidderPanno, "GSTIN Number : "+bidderGstNo, "CRN Number : "+bidderCrn);
}
				...
				if(!$.isEmptyObject(value.partner))
					{
					   ...
					   /* +'	<label class="col-xs-6" id="'+bidderId+'_crnNumber">'+crnNumber+'</label>'
					    ...
					    +'	<label class="col-xs-6" id="'+bidderId+'_panno">'+panno+'</label>'
					    +'	<label class="col-xs-6" id="'+bidderId+'_name">'+name+'</label>'
					    +'	<label class="col-xs-6" id="'+bidderId+'_bidderId">'+bidderId+'</label>'
						+' </div>'*/
					    +' </li>';
						i++;
					}
