...
function getEEUsersForChairPerson(tahdrId) {
	...
	submitWithParam('getUserForCommittee', 'tenderId', 'loadUsers');
}
...
function loadTenderCommitteeRightPane(data) {
	...
	/*setHeaderValues("Tender : " + tahdrName, "Tender Version : "
			+ tahdrVersionNumber, "Chairperson : " + chairPersionName, "");*/
}
...
function tenderCommitteeDelResp(data) {
	...
	if (data.hasError == false) {
		...
		$(".tenderCommitteeBtn").removeAttr("disabled", 'disabled');
		/*
		 * setActiveTabName("Factory Essential Details",$('.leftPaneData
		 * li').length);
		 */
...
function fetchTenderCommitteeList(pageNumber, pageSize, searchMode, searchValue,openingType,typecode){
	...
	$.ajax({
        ...
        url: "fetchTenderCommittee?pageNumber="+pageNumber+"&pageSize="+pageSize+'&searchMode='+searchMode+'&serachValue='+searchValue+'&openingType='+openingType+'&typeCode='+typecode,
        dataType:"json",
        async:false,
