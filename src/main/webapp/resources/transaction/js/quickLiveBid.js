...
function loadAuctionListForLiveBid(data){
	...
	serverTime=data.objectMap.serverTime;
...
function loadAuctionRightPane(data) {
	if (!$.isEmptyObject(data)) {
		...
		tenderId=tahdrId;
		tenderName=tenderNo;
		baseRate=data.tahdrDetail[0].estimatedCost == null ? ''
				: data.tahdrDetail[0].estimatedCost;
		tenderMinBidDiffer=minBidDiffer;
		...
		$("#tenderDetailsForm #decOrIncById").html(indianRupeesFormat(minBidDiffer));
		$("#bidderDetailForm #decOrIncById").html(indianRupeesFormat(minBidDiffer));
		
		$('#otpFormId #tenderId').val(tahdrId);
		$('#bidFormId #tahdrId').val(tahdrId);

		setHeaderValues("Tender No.: " + tenderCode, "Remaining Time : "
				, "Contact EmailId : " + contactEmail,
				"Description : " + description);

		setTimer(bidEndDate,false);
		...
		setHeaderValues("Tender No.: ", "Remaining Time : ",
...
function loadAuctionMaterialListForView(data) {
	...
	if(!isEmpty(data.objectMap.remainingTime)){
	...
	if (!$.isEmptyObject(data.objectMap.responseList)) {
		$.each(data.objectMap.responseList, function(key, value) {
			...
			var materialData=value==null?null:value;
			if(materialData!=null){
			bidderUniqueCode=Number(value.created)+Number(i*5000);
...
function autoRefreshDataView(data){
	...
	if(data.objectMap.result){
		...
		var bidDifference=Number(newLowestBidValue)-Number(mynewLowestBidValue);
		$('.detailscont #middleRightId').html(currentLabelName+" Bid: "+indianRupeesFormat(newLowestBidValue));
		$('.detailscont #bottomLeftId').html("My "+currentLabelName+" Bid : "+indianRupeesFormat(mynewLowestBidValue));
		$('.detailscont #bottomRightId').html("Bid Difference:"+indianRupeesFormat(bidDifference));
		var currentRank=data.objectMap.rank;
		setTimer(data.objectMap.remainingTime,true);
...
function loadAuctionMaterialRightPane(data,rank) {
	...
	}else if(typeCode=='QFA' || typeCode=='QRA'){
		currentLabelName=typeCode =='QFA'?'Highest':'Lowest';
		decrementOrIncrement=typeCode =='QFA'?'Increment By :':'Decrement By :';
		if( typeCode=='QRA'){
			$('#decrementBtnId').show();
			$('#incrementBtnId').hide();
		}else{
	...
	if (!$.isEmptyObject(data)) {
		$('#bidFormId #auctionEndDate').val(bidEndDate);
		...
		var totalQty = data.quantity== null ? ''
				: data.quantity;
		var baseRateValue=baseRate;
		totalQuantity=totalQty;
		$('#bidFormId #bidderOfferQty').val(totalQuantity);
		...
		$("#bidderDetailForm #decOrIncByLblId").html(decrementOrIncrement);
		...
		var currentBidValue=indianRupeesFormat(currentRate);
		var bidDiffer=Number(currentRate)-Number(g_fddRateWithGST);
		$('#bidFormId #preTotalFDDAmountWithGST').val(currentRate);
		setHeaderValuesWithMoreParam("Material Name.: " + name, " Remaining Time : ", "Reserved Price: "+indianRupeesFormat(baseRateValue) ,
				currentLabelName+" Bid: "+currentBidValue,"My "+currentLabelName+" Bid : "+indianRupeesFormat(g_fddRateWithGST)," Bid Difference: "+indianRupeesFormat(bidDiffer))
		...
		setHeaderValuesWithMoreParam("Material Name.: ", "Remaining Time : ", "Reserve Price : " ,
				" Bid (in Rs): ","My  Bid (in Rs): "," Bid Difference:" );
...
function setBidderDetail(data){
	...
	if(!$.isEmptyObject(data)){
		...
		$("#bidderDetailForm #exGroupPriceRate").html(indianRupeesFormat(exGroupPriceRate));
		...
		$("#bidderDetailForm #totalExGroupPrice").html(indianRupeesFormat(totalExGroupPrice));
...
function resetToPreviousBid(){
	...
	var tahdrMaterialId=$('.leftPaneData').find('li.active').attr('id');
	submitToURL('autoRefreshData/'+tahdrId+'/'+tahdrMaterialId,'autoRefreshDataView');*/
	$('#bidFormId #inputExGroupPriceRateId').data('times',1);
	$('#bidFormId #inputExGroupPriceRateId').val(g_fddRateWithGST);
	$('#autoRefresh').timer('reset');
}
...
function saveNewBidResp(data){
	...
	if(!$.isEmptyObject(data.objectMap)){
		if(data.objectMap.result){
			...
			currentRate=$("#bidderDetailForm #fddRateGST").html();
			var currentBidValue=Math.trunc(currentRate);
			g_fddRateWithGST=currentBidValue;
			if(data.objectMap.isAuctionExtended){
				bidEndDate=data.objectMap.newDetail==null?'':data.objectMap.newDetail.auctionToDate==null?'':data.objectMap.newDetail.auctionToDate;
				var remainingTime=data.objectMap.remainingTime==null?'0s':data.objectMap.remainingTime;
				setTimer(remainingTime,true);
			}
			$('.detailscont #middleRightId').html(currentLabelName+" Bid: "+indianRupeesFormat(currentBidValue));
			$('.detailscont #bottomLeftId').html("My "+currentLabelName+" Bid: "+indianRupeesFormat(currentBidValue));
			$('.detailscont #bottomRightId').html("Bid Difference:"+0); /*bidDifferrence*/
...
function loadSelfBidHistoryView(data){
	...
	if(!$.isEmptyObject(data.objectMap.bidHistory)){
		...
		$('.selfbidHistory tbody').append('<tr>' + 
				...
				'<td class="col-sm-2">'+indianRupeesFormat(prevFddAmountWithGST)+'</td>'+
				'<td class="col-sm-2">'+indianRupeesFormat(exGroupPriceRate)+'</td>'+
				'<td class="col-sm-2">'+indianRupeesFormat(fddAmountWithGST)+'</td>');
		});
	}
...
function bidderListResp(data){
	...
	if(!$.isEmptyObject(data.objectMap.bidHistory)){
		...
		var factoryName=value.itemBid==null?'':value.itemBid.bidder==null?'':value.itemBid.bidder.factory==null?''
				:value.itemBid.bidder.factory.name;
		var unique=Number(value.created)+Number(i*5000);
		var offeredQuantity=value.offeredQuantity==null?'':value.offeredQuantity;
		var date=value.updated==null?'':formatDateTime(value.updated);
		var exGroupPriceRate=value.exGroupPriceRate==null?'':indianRupeesFormat(value.exGroupPriceRate);
		var fddAmountWithGST=value.fddAmountWithGST==null?'':indianRupeesFormat(value.fddAmountWithGST);
		var fddRateWithGST=value.fddRateWithGST==null?'':indianRupeesFormat(value.fddRateWithGST);
