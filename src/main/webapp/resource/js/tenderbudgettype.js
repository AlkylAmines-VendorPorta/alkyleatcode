	...
	$.ajax({
        ...
        success: function (data) {
        	if($.isEmptyObject(data))
        		{
        		  swal("No TenderBudgettype present in List");
        		}
        	else
        	{
	        	appendList(data);
...
function appendList(data)
{
	...
	for (var i=0;i<data.length;i++)  { 
		...
		if(i==0)
			{
				...
				$(".Id").val(data[i].tenderBudgetTypeId);
		...
		$('#example').append('<li '+active+' onclick="showdetails('+data[i].tenderBudgetTypeId+')"> <a href="" class="" data-toggle="tab">'
		           +' <div class="col-md-12">'
		           +'  <label class="col-xs-6">'+data[i].name+'</label>'
		           +'   <label class="col-xs-6 mytext" data-tenderbudgettypeid="'+data[i].tenderBudgetTypeId+'">'+data[i].description+'</label>'
		           +'  </div>'
		           +'  <div class="col-md-12">'
...
function showdetails(id)
{
	...
	$.ajax({
        ...
        success: function (data) {
        	if($.isEmptyObject(data))
        		{
        		  swal("Tenderbudgettype details is empty-Add Details");
        		}
        	else
        	{
	        	 var activeStatus='';
