...
{ 
	...
	$.ajax({
        ...
        url: "getBusinessPartnerGroupList",
        dataType:"json",
        async:false,
        success: function (data) {
        	if($.isEmptyObject(data))
        		{
        		  swal("No Business Partner Group present in List");
        		}
        	else
        	{
	        	appendList(data);
...
function appendList(data)
{
	...
	for (var i=0;i<data.length;i++)  { 
		...
		$('#example').append('<li '+active+' onclick="showdetails('+data[i].bPartnerGroupId+')"> <a href="" class="" data-toggle="tab">'
		           +' <div class="col-md-12">'
		           +'  <label class="col-xs-6">'+data[i].name+'</label>'
		           +'   <label class="col-xs-6 mytext" data-id="'+data[i].bPartnerGroupId+'">'+data[i].name+'</label>'
		           +'  </div>'
		           +'  <div class="col-md-12">'
...
function showdetails(id)
{
	...
	$.ajax({
        type : "POST",
        contentType : "application/json",
        url: "getBusinessPartnerGroup/"+id,
        dataType:"json",
        async:false,
        success: function (data) {
        	if($.isEmptyObject(data))
        		{
        		  swal("Business Partner Group details is empty");
        		}
        	else
        	{
	        	 var activeStatus="";
