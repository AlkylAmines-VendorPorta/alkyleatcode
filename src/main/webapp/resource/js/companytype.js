	...
	$.ajax({
        ...
        success: function (data) {
        	if($.isEmptyObject(data))
        		{
        		  swal("No Companytype present in List");
        		}
        	else
        	{
...
function appendList(data)
{
	...
	for (var i=0;i<data.length;i++)  { 
		...
		$('#example').append('<li '+active+' onclick="showdetails('+data[i].companyTypeId+')"> <a href="" class="" data-toggle="tab">'
		           +' <div class="col-md-12">'
		           +'  <label class="col-xs-6">'+data[i].name+'</label>'
		           +'   <label class="col-xs-6 mytext" data-Id="'+data[i].companyTypeId+'">'+data[i].description+'</label>'
		           +'  </div>'
		           +'  <div class="col-md-12">'
...
function showdetails(id)
{
	...
	$.ajax({
        ...
        success: function (data) {
        	if($.isEmptyObject(data))
        		{
        		  swal("Companytype details is emptyc");
        		}
        	else
        	{
