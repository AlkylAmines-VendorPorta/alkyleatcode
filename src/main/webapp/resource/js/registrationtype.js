	...
	$.ajax({
        ...
        success: function (data) {
        	if($.isEmptyObject(data))
        		{
        		  swal("No Registrationtype present in List");
        		}
        	else
        	{
	        	appendList(data);
...
function appendList(data)
{
	...
	for (var i=0;i<data.length;i++)  {
		...
		if(i==0)
			{
				...
				$(".Id").val(data[i].registrationTypeId);
		...
		$('#example').append('<li '+active+' onclick="showdetails('+data[i].registrationTypeId+')"> <a href="" class="" data-toggle="tab">'
		           +' <div class="col-md-12">'
		           +'  <label class="col-xs-6">'+data[i].name+'</label>'
		           +'   <label class="col-xs-6 mytext" data-registrationtypeid="'+data[i].registrationTypeId+'">'+data[i].description+'</label>'
		           +'  </div>'
		           +'  <div class="col-md-12">'
...
function showdetails(id)
{
	...
	$.ajax({
        ...
        success: function (data) {
        	if($.isEmptyObject(data))
        		{
        		  swal("Registrationtype details is empty");
        		}
        	else
        	{
        		var activeStatus ='';
				...
				$(".Id").val(data.registrationTypeId);
				if(data.isActive=="Y")
				{
					$('.isActive').prop('checked', true);
					activeStatus="Active";
